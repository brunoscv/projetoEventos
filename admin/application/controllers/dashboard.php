...
class Dashboard extends MY_Controller {
	...
	public function index(){
		...
		// Verifica se existe consultas geradas para a data atual
		$resultConsultaGerada = $this->db
			->select("*")
			->from("frequencias as fr")
			->where(array("fr.createdAt" => date("Y-m-d"),
							"fr.status" => 1))
			->get();
		
		$this->data['listaConsultaGerada'] = $resultConsultaGerada->result();
