...
function recursiveMenuNav($menuList, $extra = ""){
	...
	$str="";
		...
		foreach($menuList as $key => $menu){
			...
			if( $hasSubmenu ){
				$str .= "<li><a href=\"#\" data-toggle=\"collapse\" data-target=\"#submenu-{$key}\"><i class=\" {$menu->icone}\"> </i><span>" ."  ". $menu->descricao."</span><span class=\"fa fa-fw fa-angle-down pull-right\"><i class=\"md md-add\"></i></span></a><ul id=\"submenu-{$key}\" class=\"collapse\">";
				$str .= recursiveMenuChildNav($menu->filhos,'class="list-unstyled" style="display:none"');
			} else {
				$str .= "<li><a href=\"{$url}\" class=\"{$classLink}\"><i class=\"menu-icon {$menu->icone}\"> </i><span>" . "  ". $menu->descricao."</span></a>";
			}
...
function recursiveMenuChildNav($menuList, $extra = "") {
	...
	foreach($menuList as $key => $menu){
		...
		$str .= "<li><a href=\"{$url}\"><i class=\"fa\"></i>" . " " . $menu->descricao."</a></li>";
